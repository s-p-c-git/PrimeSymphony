...
and a self-contained argument-principle computation confirms numerically, at
high precision, that all zeros of both functions with $|\im s|<60$ lie on the
critical line. None of the
individual facts is new---the underlying mechanisms are classical, tracing to
Erd\H{o}s, Hardy--Littlewood, Dirichlet, and Alladi---but their
...
literature. In particular, the Dirichlet-series decomposition we use is
classical---it is character orthogonality applied to the prime zeta function,
and the associated M\"obius inversion of $\log\zeta$ is standard (see, e.g.,
Titchmarsh~\cite{Titchmarsh}); cf.\ also Chavez~\cite{Chavez2023} for a recent
treatment of related decompositions. The distributional facts are folklore in
the shifted-prime-factor community.
...
\end{corollary}
\begin{proof}
The residue $-k\bmod q$ depends only on $k\bmod q$, and by
Theorem~\ref{thm:class} the splitting type depends only on that residue.
Restricting to even $k$, the pair $(k\bmod 2,\,k\bmod q)$ determines the type,
and since $q$ is odd, $\gcd(2,q)=1$, so by the Chinese Remainder Theorem the
type has period $\operatorname{lcm}(2,q)=2q$ in $k$. For $q=3$ this is period
$6$; the three even residues $k\equiv2,4,0\pmod 6$ give $-k\equiv1,2,0\pmod3$,
which are respectively a quadratic residue, a non-residue, and $0$, hence split,
inert, and empty.
\end{proof}
...
\end{corollary}
\begin{proof}
Take $k=2$, $q=3$, so $a=-2\bmod 3=1$. Since $1$ is a quadratic residue
mod~$3$, Theorem~\ref{thm:class} gives that every $p\in S_2(3)$ splits in
$\Q(\sqrt{-3})$, whose ring of integers is $\Z[\omega]$. Conversely a rational
prime $p>3$ splits in $\Z[\omega]$ exactly when $p\equiv1\pmod 3$, so the two
sets coincide. For the second claim, $p\equiv1\pmod3$ forces $3\mid p+2$ with
$p+2>3$, so $p+2$ is composite: every such $p$ is an orphan, and no twin prime
lies in the class.
\end{proof}
...
\end{theorem}
\begin{proof}
Fix $q$ and let $R=\{r \text{ prime}: 3\le r<q\}$, a finite set, and
$M=q\prod_{r\in R}r$. The condition $\spf(p+2)=q$ is equivalent to the
conjunction of $p\equiv-2\pmod q$ and $p\not\equiv-2\pmod r$ for each $r\in R$;
this is a union of residue classes modulo $M$. By the Chinese Remainder Theorem
the admissible residues $a\bmod M$ are exactly those with $a\equiv-2\pmod q$ and
$a\not\equiv-2\pmod r$ for all $r\in R$, and among the $\varphi(M)$ classes
coprime to $M$ their number is
\[
  \#\{a\} \;=\; \frac{\varphi(M)}{q-1}\prod_{r\in R}\frac{r-2}{r-1},
\]
since modulo $q$ exactly one of the $q-1$ coprime classes is $-2$, and modulo
$r$ exactly one of the $r-1$ coprime classes is excluded. Dirichlet's theorem in
the quantitative form (the prime number theorem for arithmetic progressions)
gives each coprime class modulo $M$ the same density $1/\varphi(M)$ among the
primes. Summing over the admissible classes yields the stated density. As $M$ is
fixed once $q$ is, no uniformity in $q$ is required and the argument is
unconditional.
\end{proof}

Over the $348{,}510$ primes below $5\times10^6$ the formula matches the measured
fractions to within $1\%$ for every $q\le 41$.

Summing over $q$ recovers the full density, as it must: for $p>3$ the value
$p+2$ is either prime or composite, and in the composite case $\spf(p+2)$ is
some odd prime. By Brun's theorem the twin primes have density $O(1/\log x)$
among the primes, hence density zero, so
\[
  \sum_{q\ \mathrm{odd\ prime}} \frac{1}{q-1}
  \prod_{\substack{r\ \mathrm{prime}\\ 3\le r<q}}\frac{r-2}{r-1} \;=\; 1 .
\]
Convergence is slow. Writing $P(q)=\prod_{3\le r<q}(r-2)/(r-1)$ and using
$(r-2)/(r-1) = (1-1/r)(1-1/(r-1)^2)$, Mertens' theorem over the odd primes
together with the twin-prime product $C_2=\prod_{r>2}(1-1/(r-1)^2)$ gives
\[
  P(q) \sim \frac{2e^{-\gamma}C_2}{\log q}, \qquad
  2e^{-\gamma}C_2 = 0.74131\ldots,
\]
so the summand is $\asymp 1/(q\log q)$ and the tail beyond $Q$ is
$\sim 2e^{-\gamma}C_2/\log Q$. Numerically: the partial sums to
$q<2\times10^3$, $2\times10^5$, $5\times10^5$ are $0.9027$, $0.9393$, $0.9450$,
and adding the predicted tail gives $1.0001$, $0.9999$, $0.9999$ respectively.
...
$\re(s)>\tfrac12$). Equation~\eqref{eq:decomp} is classical: it is character
orthogonality mod $3$ combined with the standard expansion
$\log L(s,\chi)=\sum_p\chi(p)p^{-s}+O(1)$ on $\re(s)>\tfrac12$; equivalently it
reflects the Dedekind factorisation
$\zeta_{\Q(\sqrt{-3})}(s)=\zeta(s)L(s,\chi_{-3})$. Cf.\
Chavez~\cite{Chavez2023} for related decompositions of prime $L$-series.
...
The same zeros can also be \emph{counted}. Figure~\ref{fig:heatmap}
already shows where each function is small in the strip: every dark valley
sits exactly on $\re(s)=\tfrac12$. The argument principle makes this
quantitative: for the rectangle $\Omega=\{0.1<\re s<0.9,\ |\im s|<60\}$, the
winding number around $\partial\Omega$---sampled at $400$ points per side and
evaluated at $25$-digit precision---returns $26.00000$ for $\zeta$ and
$44.00000$ for $L(\cdot,\chi_{-3})$, with $\min|f|$ on the contour bounded
below by $0.27$ and $0.12$ respectively. These match the $2\times 13$ and
$2\times 22$ on-line zeros located independently by sign changes
(cross-checked for $\zeta$ against the standard count), which is consistent
with every zero of both functions in $\Omega$ being simple and on
$\re(s)=\tfrac12$.

We are deliberate about the status of this computation. It is a numerical
verification at high precision, not a certified proof: a fully rigorous
version requires interval arithmetic with adaptive refinement, guaranteeing
that no phase change is missed between consecutive contour samples. That is
exactly the machinery Platt and Trudgian~\cite{PlattTrudgian2021} deploy in
verifying the Riemann hypothesis to height $3\times10^{12}$ by Turing's
method. What is offered here is the same argument in miniature, at a
precision that leaves the integer counts unambiguous, but without the
certification that would license the word ``proof''.
