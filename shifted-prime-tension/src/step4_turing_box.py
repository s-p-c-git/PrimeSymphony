"""
Step 4 — Turing's-method box verification (high-precision numerical).

Supports: §5. NOT a certified proof — see step4b_phase_analysis.py.
...
the on-line zero counts, this is CONSISTENT WITH every zero of both functions
in the box being simple and lying on Re(s)=1/2. The contour is sampled, so a
phase change between consecutive samples is not excluded; that is why this is a
numerical verification rather than a proof.
...
print("VERDICT (numerical verification, not a certified proof; see §5):")
print(f"  zeta:      {Wz:.4f} vs 26 -> {'CONSISTENT with on-line' if abs(Wz-26)<0.01 else 'MISMATCH'}")
print(f"  L(chi_-3): {Wl:.4f} vs 44 -> {'CONSISTENT with on-line' if abs(Wl-44)<0.01 else 'MISMATCH'}")
